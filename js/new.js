...
// ADDING THE ITEMS TO THE LISTL
var canIReallyANumber = 0;
...
function marketSearch(query)
{
    ...
    resultsCount = resultsCount > 100 ? 100 :  resultsCount;
    ...
    if(query.length > 0){
        ...
        for (var x = 0; x < resultsCount; x++){
            if(x == 0){
                ...
                quantSele[x] = quantSele[x-1] + 9;
            }
        }


        // STANDARD AJAX STUFF
        $.get("https://still-mountain-53636.herokuapp.com/https://steamcommunity.com/market/search/render/?query="+query+"&count="+resultsCount, function(){
        }).done(function(response){
            var response = response;
            var selector = document.getElementById("results");
            ...
            selector.appendChild(br1);
            selector.appendChild(br2);
            selector.appendChild(hr);
            selector.appendChild(ul);

            // DISPLAYING THE RESULTS
            parser = new DOMParser();
            doc = parser.parseFromString(response.results_html, 'text/html');
            if(response.total_count == 0){
                ...
                for (var i = 0; i < resultsCount; ++i){
                    ...
                    links[i] = doc.getElementById("resultlink_"+i).href;
                    names[i] = doc.getElementById("result_"+i+"_name").innerHTML;
                    quant[i] = doc.getElementsByTagName('span')[quantSele[i]].innerHTML;
                    starting[i] = doc.getElementsByTagName('span')[quantSele[i]+2].innerHTML;
                    sale[i] = doc.getElementsByTagName('span')[quantSele[i]+3].innerHTML;
                    itemnames[i] = links[i].substr(47);
                    if(itemnames[i].indexOf('?filter') !== -1){
                        itemnames[i] = itemnames[i].substring(0, itemnames[i].indexOf("?"));
                    }
                    appids[i] = links[i].substr(43,3);
                }
                ...
                for(var i = 0; i < imgs.length; i++){
                    ...
                    $('<img/>',{
                        src: imgs[i]
                    }).appendTo('#results > ul > li:nth-of-type('+x+')');
                    $('<span/>',{
                        href: links[i],
                        text: names[i]
                    }).appendTo('#results > ul > li:nth-of-type('+x+')');
                    ...
                    $('<span/>',{
                        text: 'Starting price: '+starting[i]
                    }).appendTo('#results > ul > li:nth-of-type('+x+')');
                    $('<span/>',{
                        text: 'Sale price: '+sale[i]
                    }).appendTo('#results > ul > li:nth-of-type('+x+')');
                    $('<button/>',{
                        name: itemnames[i],
                        class: 'btn pull-right '+appids[i]+' btn-default',
                        type: 'button',
                        text: '+',
                        onclick: 'printItem(this.name,this.classList[2])',
                    }).appendTo('#results > ul >li:nth-of-type('+x+')');
                    $('<hr/>').appendTo('#results ul');
                }
            }
...
{
    $.get("https://still-mountain-53636.herokuapp.com/https://steamcommunity.com/market/priceoverview/?appid="+appid+"&currency="+currency+"&market_hash_name="+name, function(){
        }).done(function(response){
            // DISPLAYING THE PRICE
            var newElement = document.getElementsByName(number);
            newElement = newElement[0].childNodes[4];
            newElement.innerHTML = response.lowest_price;
        }).fail(function(jqXHR, textStatus, error){
            document.getElementsByName(number).innerHTML = "Timed out.";
        });
